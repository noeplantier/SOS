...
export class ContactService {
  ...
  async getVehicleById(vehicleId: string): Promise<Vehicle | null> {
    try {
      const response = await axios.get(`${this.apiUrl}/vehicles/${vehicleId}`, {
        headers: {
  ...
  async getAvailableContacts(): Promise<Contact[]> {
    try {
      const response = await axios.get(`${this.apiUrl}/contacts?isAvailable=true`, {
        headers: {
          'Authorization': `Bearer ${this.apiKey}`
        }
      });
  ...
  async findContactsInZone(location: string): Promise<Contact[]> {
    try {
      const response = await axios.get(`${this.apiUrl}/contacts/zone?location=${encodeURIComponent(location)}`, {
        headers: {
          'Authorization': `Bearer ${this.apiKey}`
        }
      });
  ...
  async selectContactsForEmergency(criteria: ContactSelectionCriteria): Promise<Contact[]> {
    try {
      ...
      if (criteria.vehicleId) {
        ...
        if (vehicle && vehicle.emergencyContacts && vehicle.emergencyContacts.length > 0) {
          ...
          selectedContacts = contacts.filter(contact => contact !== null) as Contact[];
        }
      }
      ...
      if (selectedContacts.length < 3) {
        ...
        const specializedContacts = availableZoneContacts.filter(contact => 
          contact.specialties.includes(criteria.emergencyType)
        );
        ...
        for (const contact of specializedContacts) {
          if (!selectedContacts.some(c => c.id === contact.id)) {
            ...
            if (selectedContacts.length >= 5) break; // Limiter à 5 contacts max
          }
        }
