...
var AirtableV1_node_exports = {};
__export(AirtableV1_node_exports, {
  AirtableV1: () => AirtableV1
});
module.exports = __toCommonJS(AirtableV1_node_exports);
...
class AirtableV1 {
  ...
  async execute() {
    ...
    const application = this.getNodeParameter("application", 0, void 0, {
      extractValue: true
    });
    const table = encodeURI(
      this.getNodeParameter("table", 0, void 0, {
        extractValue: true
      })
    );
    let returnAll = false;
    ...
    if (operation === "append") {
      ...
      try {
        ...
        for (const key of Object.keys(additionalOptions)) {
          if (key === "sort" && additionalOptions.sort.property !== void 0) {
      ...
      for (let i = 0; i < items.length; i++) {
        try {
          ...
          if (updateAllFields) {
            ...
            if (options.ignoreFields && options.ignoreFields !== "") {
              ...
              if (ignoreFields.length) {
                row.fields = Object.entries(items[i].json).filter(([key]) => !ignoreFields.includes(key)).reduce((obj, [key, val]) => Object.assign(obj, { [key]: val }), {});
              }
            }
          } else {
            fields = this.getNodeParameter("fields", i, []);
          ...
          if (rows.length === bulkSize || i === items.length - 1) {
            ...
            responseData = await import_GenericFunctions.apiRequest.call(this, requestMethod, endpoint, data, qs);
            const executionData = this.helpers.constructExecutionMetaData(
              this.helpers.returnJsonArray(responseData.records),
              { itemData: { item: i } }
            );
