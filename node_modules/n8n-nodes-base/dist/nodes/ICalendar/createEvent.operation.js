...
var createEvent_operation_exports = {};
__export(createEvent_operation_exports, {
  description: () => description,
  execute: () => execute
});
module.exports = __toCommonJS(createEvent_operation_exports);
var ics = __toESM(require("ics"));
var import_moment_timezone = __toESM(require("moment-timezone"));
var import_n8n_workflow = require("n8n-workflow");
var import_util = require("util");
const createEvent = (0, import_util.promisify)(ics.createEvent);
const description = [
  {
...
async function execute(items) {
  ...
  for (let i = 0; i < items.length; i++) {
    try {
      ...
      end = allDay ? (0, import_moment_timezone.default)(end).utc().add(1, "day").format() : end;
      const binaryPropertyName = this.getNodeParameter("binaryPropertyName", i);
      const options = this.getNodeParameter("additionalFields", i);
      if (options.useWorkflowTimezone) {
        start = (0, import_moment_timezone.default)(start).tz(workflowTimezone).format();
        end = (0, import_moment_timezone.default)(end).tz(workflowTimezone).format();
        delete options.useWorkflowTimezone;
      }
      let fileName = "event.ics";
      const eventStart = (0, import_moment_timezone.default)(start).toArray().splice(0, allDay ? 3 : 6);
      eventStart[1]++;
      const eventEnd = (0, import_moment_timezone.default)(end).toArray().splice(0, allDay ? 3 : 6);
      eventEnd[1]++;
      if (options.fileName) {
