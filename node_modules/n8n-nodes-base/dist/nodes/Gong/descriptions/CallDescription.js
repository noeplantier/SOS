              ...
              async function(items, _responseData) {
                ...
                if (properties.includes("transcript")) {
                  for (const item of items) {
                    const callTranscripts = await import_GenericFunctions.gongApiPaginateRequest.call(
                      this,
                      "POST",
                      "/v2/calls/transcript",
                      { filter: { callIds: [item.json.metaData.id] } },
                      {},
                      item.index ?? 0,
                      "callTranscripts"
                    );
                    item.json.transcript = callTranscripts?.length ? callTranscripts[0].transcript : [];
                  }
                }
                return items;
              }
...
const getAllFields = [
  {
    ...
    routing: {
      ...
      operations: {
        pagination: (0, import_GenericFunctions.getCursorPaginatorCalls)()
      }
    },
    type: "boolean"
  },
      ...
      output: {
        postReceive: [
          async function(items, _response) {
            return (0, import_GenericFunctions.extractCalls)(items);
          },
          {
            type: "limit",
            properties: {
          ...
          send: {
            preSend: [
              async function(requestOptions) {
                ...
                if (callIdsParam && !(0, import_GenericFunctions.isValidNumberIds)(callIdsParam)) {
                ...
                const callIds = Array.isArray(callIdsParam) ? callIdsParam.map((x) => x.toString()) : callIdsParam.toString().split(",").map((x) => x.trim());
                requestOptions.body ||= {};
...
const callFields = [...getFields, ...getAllFields];
