...
__export(GenericFunctions_exports, {
  getActionInheritedProperties: () => getActionInheritedProperties,
  ...
  webexApiRequest: () => webexApiRequest,
  webexApiRequestAllItems: () => webexApiRequestAllItems
});
  ...
  for (const resource of Object.keys(resourceEvents)) {
    elements.push({
      ...
      options: resourceEvents[resource].map((event) => ({
        ...
        name: (0, import_upperFirst.default)(event)
      })),
      default: "",
      required: true
    });
...
function removeEmptyProperties(rest) {
  return Object.keys(rest).filter((k) => rest[k] !== "").reduce((a, k) => ({ ...a, [k]: rest[k] }), {});
}
function getAttachments(attachments) {
  ...
  for (const attachment of attachments) {
    const body = [];
    const actions = [];
    for (const element of (attachment?.elementsUi).elementValues || []) {
      const { type, ...rest } = element;
      if (type.startsWith("input")) {
        body.push({
          type: `Input.${(0, import_upperFirst.default)(type.replace("input", ""))}`,
          ...removeEmptyProperties(rest)
        });
      } else {
        body.push({ type: (0, import_upperFirst.default)(type), ...removeEmptyProperties(rest) });
      }
    }
    for (const action of (attachment?.actionsUi).actionValues || []) {
      const { type, ...rest } = action;
      actions.push({ type: `Action.${(0, import_upperFirst.default)(type)}`, ...removeEmptyProperties(rest) });
    }
    _attachments.push({
