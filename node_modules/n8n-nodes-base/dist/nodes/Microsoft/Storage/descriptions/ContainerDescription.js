        ...
        routing: {
          ...
          output: {
            postReceive: [
              import_GenericFunctions.handleErrorPostReceive,
              async function(_data, response) {
                const { metadata, ...properties } = (0, import_GenericFunctions.parseHeaders)(response.headers);
                ...
                return [
                  {
                    json: {
                      name: this.getNodeParameter("container").value,
                      properties,
                      ...metadata ? { metadata } : {}
                    }
                  }
          ...
          output: {
            postReceive: [
              ...
              async function(data, _response) {
                return [
                  {
                    json: await (0, import_GenericFunctions.parseContainerList)(data[0].json)
                  }
                ];
              }
            ]
      ...
      send: {
        preSend: [
          async function(requestOptions) {
            const container = this.getNodeParameter("containerCreate");
    ...
    options: [
      {
        ...
        routing: {
          request: {
            headers: {
              [import_GenericFunctions.HeaderConstants.X_MS_BLOB_PUBLIC_ACCESS]: "={{ $value || undefined }}"
            }
          }
        },
        type: "options",
        ...
        async pagination(requestOptions) {
          ...
          do {
            ...
            executions = executions.concat(
              responseData[0].json.containers.map((item) => ({ json: item }))
            );
          } while (marker);
          return executions;
        }
  ...
  },
  {
    ...
    routing: {
      ...
      output: {
        postReceive: [
          async function(data, _response) {
            return data[0].json.containers.map((item) => ({ json: item }));
          }
        ]
      }
    },
