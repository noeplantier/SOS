            ...
            for (const item of items) {
              ...
              if (Object.keys(fields).length) {
                ...
                if (body.employeeLeaveDateTime) {
                  body.employeeLeaveDateTime = body.employeeLeaveDateTime.toUTC().toISO();
                }
                ...
                if (body.forceChangePassword) {
                  if (body.forceChangePassword === "forceChangePasswordNextSignIn") {
                    ...
                    body.passwordProfile ??= {};
                    body.passwordProfile.forceChangePasswordNextSignInWithMfa = true;
                  }
                ...
                try {
                  if (Object.keys(separateBody).length) {
                    await import_GenericFunctions.microsoftApiRequest.call(this, "PATCH", `/users/${userId}`, separateBody);
                    (0, import_lodash.merge)(item.json, separateBody);
                  }
                  if (Object.keys(body).length) {
                    await import_GenericFunctions.microsoftApiRequest.call(this, "PATCH", `/users/${userId}`, body);
                    (0, import_lodash.merge)(item.json, body);
                  }
                } catch (error) {
                  try {
                    await import_GenericFunctions.microsoftApiRequest.call(this, "DELETE", `/users/${userId}`);
                  } catch {
                  }
                  throw error;
                }
        ...
        postReceive: [
          async function(items, _response) {
            for (const item of items) {
              ...
              const separateFields = Object.keys(fields).filter((key) => separateProperties.includes(key)).reduce((obj, key) => {
                return {
                  ...obj,
                  [key]: fields[key]
                };
