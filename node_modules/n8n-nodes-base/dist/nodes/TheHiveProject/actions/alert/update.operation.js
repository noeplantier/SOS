...
async function execute(i, item) {
  ...
  if (fieldsToMatchOn.includes("id")) {
    await import_transport.theHiveApiRequest.call(this, "PATCH", `/v1/alert/${id}`, updateBody);
  } else {
    const filter = {
      _name: "filter",
    ...
    updated = ids.length;
    updateBody.ids = ids;
    await import_transport.theHiveApiRequest.call(this, "PATCH", "/v1/alert/_bulk", updateBody);
  }
  const executionData = this.helpers.constructExecutionMetaData(
    (0, import_utilities.wrapData)({ success: true, updatedAlerts: updated }),
    {
      itemData: { item: i }
    }
  );
