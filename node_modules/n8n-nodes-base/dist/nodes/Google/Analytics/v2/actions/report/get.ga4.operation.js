...
var get_ga4_operation_exports = {};
__export(get_ga4_operation_exports, {
  description: () => description,
  execute: () => execute
});
module.exports = __toCommonJS(get_ga4_operation_exports);
var import_FiltersDescription = require("./FiltersDescription");
...
const description = [
    ...
    options: [
      {
        ...
        name: "metricValues",
        values: [
          ...import_FiltersDescription.metricDropdown,
          {
    ...
    options: [
      {
        ...
        values: [...import_FiltersDescription.dimensionDropdown]
      }
    ],
    displayOptions: {
      show: {
  ...
  },
  {
    ...
    options: [
      ...
      ...import_FiltersDescription.dimensionFilterField,
      {
      ...
      ...import_FiltersDescription.metricsFilterField,
      {
...
async function execute(index) {
  const propertyId = this.getNodeParameter("propertyId", index, void 0, {
    extractValue: true
  });
  ...
  if (metricsGA4.metricValues) {
    ...
    if (metrics.length) {
      import_utils.checkDuplicates.call(this, metrics, "name", "metrics");
    ...
    if (expression) {
      body.dimensionFilter = {
        [filterExpressionType]: {
          expressions: (0, import_utils.processFilters)(expression)
        }
      };
    }
  }
  if (additionalFields.metricsFiltersUI) {
    const { filterExpressionType, expression } = additionalFields.metricsFiltersUI.filterExpressions;
    if (expression) {
      body.metricFilter = {
        [filterExpressionType]: {
          expressions: (0, import_utils.processFilters)(expression)
  ...
  if (additionalFields.orderByUI) {
    let orderBys = [];
    const metricOrderBy = additionalFields.orderByUI.metricOrderBy;
    const dimmensionOrderBy = additionalFields.orderByUI.dimmensionOrderBy;
    if (metricOrderBy) {
  ...
  if (returnAll) {
    responseData = await import_transport.googleApiRequestAllItems.call(this, "", method, endpoint, body, qs);
  } else {
    body.limit = this.getNodeParameter("limit", 0);
    responseData = [await import_transport.googleApiRequest.call(this, method, endpoint, body, qs)];
  }
  if (responseData?.length && simple) {
    responseData = (0, import_utils.simplifyGA4)(responseData[0]);
  }
