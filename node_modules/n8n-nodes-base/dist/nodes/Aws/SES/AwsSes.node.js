...
var AwsSes_node_exports = {};
__export(AwsSes_node_exports, {
  AwsSes: () => AwsSes
});
module.exports = __toCommonJS(AwsSes_node_exports);
  ...
  constructor() {
    ...
    this.methods = {
      loadOptions: {
        ...
        async getTemplates() {
          const returnData = [];
          const templates = await import_GenericFunctions.awsApiRequestSOAPAllItems.call(
            this,
        ...
        if (resource === "customVerificationEmail") {
          if (operation === "create") {
            ...
            responseData = await import_GenericFunctions.awsApiRequestSOAP.call(
              ...
              "POST",
              "",
              import_node_querystring.default.stringify({
          ...
          if (operation === "send") {
            ...
            if (additionalFields.replyToAddresses) {
              setParameter(
                ...
                additionalFields.replyToAddresses
              );
            }
        ...
        if (resource === "template") {
          if (operation === "create") {
            ...
            if (additionalFields.textPart) {
              params.push(
                `Template.TextPart=${encodeURIComponent(additionalFields.textPart)}`
              );
            }
            responseData = await import_GenericFunctions.awsApiRequestSOAP.call(
              this,
          ...
          if (operation === "update") {
            ...
            if (updateFields.textPart) {
              params.push(
                `Template.TextPart=${encodeURIComponent(updateFields.textPart)}`
              );
            }
            if (updateFields.subjectPart) {
              params.push(
                `Template.SubjectPart=${encodeURIComponent(updateFields.subjectPart)}`
              );
            }
            if (updateFields.subjectPart) {
              params.push(
                `Template.HtmlPart=${encodeURIComponent(updateFields.htmlPart)}`
              );
            }
            responseData = await import_GenericFunctions.awsApiRequestSOAP.call(
              this,
