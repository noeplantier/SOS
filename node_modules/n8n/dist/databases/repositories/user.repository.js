...
let UserRepository = class UserRepository extends typeorm_1.Repository {
    constructor(dataSource) {
        super(db_1.User, dataSource.manager);
    }
    ...
    async getByIds(transaction, ids) {
        return await transaction.find(db_1.User, { where: { id: (0, typeorm_1.In)(ids) } });
    }
    ...
    async createUserWithProject(user, transactionManager) {
        const createInner = async (entityManager) => {
            const newUser = entityManager.create(db_1.User, user);
            const savedUser = await entityManager.save(newUser);
            const savedProject = await entityManager.save(entityManager.create(db_1.Project, {
                type: 'personal',
                name: savedUser.createPersonalProjectName(),
            }));
            await entityManager.save(entityManager.create(db_1.ProjectRelation, {
