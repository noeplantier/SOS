...
let FolderRepository = class FolderRepository extends typeorm_1.Repository {
    constructor(dataSource) {
        super(db_1.Folder, dataSource.manager);
    }
    ...
    addRelationFields(query, selections, select) {
        ...
        if (select?.subFolderCount) {
            if (!query.hasRelation(db_1.Folder, 'folder.parentFolder')) {
    ...
    createTagsSubQuery(query, tags) {
        return query
            .subQuery()
            .select('ft.folderId')
            .from(db_1.FolderTagMapping, 'ft')
            .innerJoin(db_1.TagEntity, 'filter_tags', 'filter_tags.id = ft.tagId')
    ...
    async findOneOrFailFolderInProject(folderId, projectId, em) {
        ...
        return await manager.findOneOrFail(db_1.Folder, {
            where: {
                id: folderId,
                homeProject: {
                    id: projectId,
    ...
    async moveAllToFolder(fromFolderId, toFolderId, tx) {
        await tx.update(db_1.Folder, { parentFolder: { id: fromFolderId } }, {
    ...
    async transferAllFoldersToProject(fromProjectId, toProjectId, tx) {
        ...
        return await manager.update(db_1.Folder, {
