...
let SharedCredentialsRepository = class SharedCredentialsRepository extends typeorm_1.Repository {
    constructor(dataSource) {
        super(db_1.SharedCredentials, dataSource.manager);
    }
    ...
    async makeOwner(credentialIds, projectId, trx) {
        ...
        return await trx.upsert(db_1.SharedCredentials, credentialIds.map((credentialsId) => ({
            projectId,
            credentialsId,
            role: 'credential:owner',
    ...
    async deleteByIds(sharedCredentialsIds, projectId, trx) {
        trx = trx ?? this.manager;
        return await trx.delete(db_1.SharedCredentials, {
            projectId,
    ...
    async findCredentialsWithOptions(where = {}, trx) {
        trx = trx ?? this.manager;
        return await trx.find(db_1.SharedCredentials, {
            where,
            relations: {
                credentials: {
