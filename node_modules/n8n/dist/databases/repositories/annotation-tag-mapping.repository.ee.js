...
let AnnotationTagMappingRepository = class AnnotationTagMappingRepository extends typeorm_1.Repository {
    constructor(dataSource) {
        super(db_1.AnnotationTagMapping, dataSource.manager);
    }
    async overwriteTags(annotationId, tagIds) {
        return await this.manager.transaction(async (tx) => {
            await tx.delete(db_1.AnnotationTagMapping, { annotationId });
            const tagMappings = tagIds.map((tagId) => ({
                annotationId,
                tagId,
            }));
            return await tx.insert(db_1.AnnotationTagMapping, tagMappings);
        });
    }
};
