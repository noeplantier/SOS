...
module.exports = {
    deleteExecution: [
        ...
        async (req, res) => {
            ...
            const { id } = req.params;
            const execution = await di_1.Container.get(execution_repository_1.ExecutionRepository).getExecutionInWorkflowsForPublicApi(id, sharedWorkflowsIds, false);
            if (!execution) {
                return res.status(404).json({ message: 'Not Found' });
            }
    ...
    getExecution: [
        ...
        async (req, res) => {
            ...
            const execution = await di_1.Container.get(execution_repository_1.ExecutionRepository).getExecutionInWorkflowsForPublicApi(id, sharedWorkflowsIds, includeData);
            if (!execution) {
                return res.status(404).json({ message: 'Not Found' });
            }
    ...
    getExecutions: [
        ...
        async (req, res) => {
            ...
            const executions = await di_1.Container.get(execution_repository_1.ExecutionRepository).getExecutionsForPublicApi(filters);
            const newLastId = !executions.length ? '0' : executions.slice(-1)[0].id;
            filters.lastId = newLastId;
            const count = await di_1.Container.get(execution_repository_1.ExecutionRepository).getExecutionsCountForPublicApi(filters);
