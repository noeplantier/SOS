...
let TaskBrokerWsServer = class TaskBrokerWsServer {
        ...
        this.heartbeatTimer = setInterval(() => {
            for (const [runnerId, connection] of this.runnerConnections.entries()) {
                if (!connection.isAlive) {
                    void this.removeConnection(runnerId, 'failed-heartbeat-check', constants_1.WsStatusCodes.CloseNoStatus);
    ...
    async removeConnection(id, reason = 'unknown', code = constants_1.WsStatusCodes.CloseNormal) {
    ...
    async stopConnectedRunners() {
        await Promise.all(Array.from(this.runnerConnections.keys()).map(async (id) => await this.removeConnection(id, 'shutting-down', constants_1.WsStatusCodes.CloseGoingAway)));
    }
};
