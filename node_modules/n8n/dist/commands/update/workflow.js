...
class UpdateWorkflowCommand extends base_command_1.BaseCommand {
    async run() {
        ...
        if (flags.id) {
            ...
            await di_1.Container.get(workflow_repository_1.WorkflowRepository).updateActiveState(flags.id, newState);
        }
        else {
            this.logger.info(`${action} all workflows`);
            if (newState) {
                await di_1.Container.get(workflow_repository_1.WorkflowRepository).activateAll();
            }
            else {
                await di_1.Container.get(workflow_repository_1.WorkflowRepository).deactivateAll();
            }
        }
