...
let ExternalSecretsService = class ExternalSecretsService {
    getProvider(providerName) {
        const providerAndSettings = di_1.Container.get(external_secrets_manager_ee_1.ExternalSecretsManager).getProviderWithSettings(providerName);
    ...
    async saveProviderSettings(providerName, data, userId) {
        ...
        await di_1.Container.get(external_secrets_manager_ee_1.ExternalSecretsManager).setProviderSettings(providerName, newData, userId);
    }
    async saveProviderConnected(providerName, connected) {
        await di_1.Container.get(external_secrets_manager_ee_1.ExternalSecretsManager).setProviderConnected(providerName, connected);
        return this.getProvider(providerName);
    }
    getAllSecrets() {
        return di_1.Container.get(external_secrets_manager_ee_1.ExternalSecretsManager).getAllSecretNames();
    }
    async testProviderSettings(providerName, data) {
        const providerAndSettings = di_1.Container.get(external_secrets_manager_ee_1.ExternalSecretsManager).getProviderWithSettings(providerName);
        const { settings } = providerAndSettings;
        const newData = this.unredact(data, settings.settings);
        return await di_1.Container.get(external_secrets_manager_ee_1.ExternalSecretsManager).testProviderSettings(providerName, newData);
    }
    async updateProvider(providerName) {
        return await di_1.Container.get(external_secrets_manager_ee_1.ExternalSecretsManager).updateProvider(providerName);
    }
};
